...
const ApplyRecord: React.FC = () => {
  ...
  const fetchGoods = async () => {
    ...
    try {
      ...
      // 过滤掉可申领数量小于等于0的物品
      const currGoods = items.filter(
        (item: { onlineQuantity: number }) => item.onlineQuantity > 0
      );
      setGoods(currGoods);
    } catch (error) {
  ...
  const filteredGoods = goods.filter(
    (g) => g.productName && g.productName.includes(search)
  );
  ...
  const handleAddConfirm = () => {
    ...
    if (exist) {
      newCart = cart.map((c) =>
        c.itemId === addItem.itemId
          ? { ...c, num: c.num + addNum, location: addLocation }
          : c
      );
    } else {
          ...
          <Button
            ...
            icon={
              <Badge count={cart.length} size="small">
                <ShoppingCartOutlined twoToneColor="#e22020" />
              </Badge>
            }
            style={{ width: "100%" }}
            ...
            renderItem={(item) => (
              <List.Item
                actions={[
                  ...
                  <Popconfirm
                    title="确定移除该物品？"
                    onConfirm={() => handleRemove(item.itemId)}
                  >
              ...
              >
                <List.Item.Meta
                  ...
                  description={
                    <div>
                      <div>
                        单价：{item.price}元/{item.unit}{" "}
                      </div>
                      <div>库存数量:{item.onlineQuantity}</div>
                    </div>
                  }
                />
              </List.Item>
