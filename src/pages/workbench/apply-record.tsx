        ...
        renderItem={item => (
          ...
          >
            <div style={{ width: 60, height: 60, background: '#f5f5f5', borderRadius: 8, marginRight: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <img src={`${item.coverImageUrl}`} alt="物品" style={{ width: 48, height: 48 }} />
            </div>
            <div style={{ flex: 1, fontSize: 12, color: '#999' }}>
              <div style={{ fontWeight: 500, color: '#111' }}>{item.productName}</div>
              <div>规格:{item.spec || '无'}</div>
              <div>单价:¥{item.price || 0}元/{item.unit || '件'}</div>
              <div>品牌：{item.brandName || '无'}</div>
              <div>供应商：{item.supplierName || '无'}</div>
              <div>可申领数量:{item.onlineQuantity || 0}</div>
            </div>
